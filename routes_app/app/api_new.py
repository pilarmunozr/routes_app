...
def create_route(payload: RouteCreate):
    ...
    if payload.plannedStartDate >= payload.plannedEndDate:
        raise HTTPException(
            status_code=400, 
            detail="Las fechas del trayecto no son válidas"
        )
        ...
        with conn.cursor() as cur:
            try:
                ...
                if "duplicate key" in str(e).lower() or "unique" in str(e).lower():
                    raise HTTPException(status_code=400, detail="El flightId ya existe")
...
def delete_route(route_id: str):
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            return {"status": "ok", "message": "Trayecto eliminado"}
